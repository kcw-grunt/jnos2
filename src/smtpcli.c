 ...
 * JNOS 2.0
 *
 * $Id: smtpcli.c,v 1.1 2015/04/22 01:51:45 root Exp root $
 *
...
#endif
    // log (-1, "dest [%s] resolve_mx -> %d", dest, destaddr);
#ifdef SMTPTRACE
        if (Smtptrace > 6)
...
/* 04Dec2014, If dest addr is BAD, then why the heck should we gateway it ? */
    ...
    // log (-1, "final destination -> %d", destaddr);
