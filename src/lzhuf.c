...
/*   short b_checksum;                         // buffer checksum.*/
   ...
   // Encode code.
	...
	{
		logmbox (usock, f->m->name, "lzhuf compress %ld/%ld = %ld percent",
			f->lzhuf->codesize, f->lzhuf->iFileSize,
				(f->lzhuf->iFileSize - f->lzhuf->codesize) * 100L / f->lzhuf->iFileSize);
	}
...
/*	pscurproc ();   13Oct2009, Maiko, Log (debug) stack utilization */
	 ...
	 */
		...
		{
			logmbox (usock, f->m->name, "lzhuf uncompress %ld/%ld = %ld percent",
				f->lzhuf->iFileSize, count,
					(count - f->lzhuf->iFileSize) * 100L / count);
		}
